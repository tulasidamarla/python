...
```
- The program `sentinel.py` can take input sequence from a file `numbers.txt`(if the file has multiple values in different rows) and write the print statements output to another file, use the below command.
```
